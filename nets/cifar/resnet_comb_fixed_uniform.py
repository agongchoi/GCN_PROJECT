...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, num_classes, num_partitionings, include_normal_head=False, use_bias=False):
        ...
        self.fc = nn.Linear(64 * block.expansion, num_classes * num_partitionings)
    ...
    def _sample_partitionings(self, k_dist):
        num_partitionings = self.num_partitionings - 1 if self.include_normal_head else self.num_partitionings
        ...
        if self.include_normal_head:
            self.partitionings[-1] = torch.arange(self.num_classes) + (num_partitionings * self.num_classes)
    ...
    def forward(self, x):
        ...
        outputs = self.fc(x).view(-1, self.num_partitionings, self.num_classes)
        ...
        if self.include_normal_head:
            normal_head_output = outputs[:, -1]
            other_head_output = outputs[:, :-1].sum(1)
