...
def build_set(root, year, train, subsample, num_train_splits, target):
    """
    ...
    if year == 2010:
        ...
        if target == -1:
            imgs = [(x, y // 2, True if y % 2 == 0 else False) for x, y in imgs]
        else:
            imgs = [(x, y // 2, False if y % 2 == 0 else True) for x, y in imgs]

            imgs = [(x, y // num_train_splits, True if y % num_train_splits == target else False) for x, y in imgs]
...
class CUB200(data.Dataset):
    ...
    def __init__(self, root, year, train=True, transform=None, target_transform=None, download=False,
                 loader=default_loader, subsample=1, num_train_splits=5, target=-1):
        ...
        self.imgs, self.classes, self.class_to_idx = build_set(os.path.join(self.root, self.raw_folder),
                                                               self.year, self.train, subsample,
                                                               num_train_splits, target)
