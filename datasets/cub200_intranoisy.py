...
def build_random_meta_class_set_per_batch(num_class, num_partitions):
    """uniform random meta-class sets per batch
    """
    random_dist = np.ones(num_partitions) / num_partitions
    while True:
        label_dist = np.random.multinomial(1, random_dist, num_class).argmax(1)
        random_meta_class_set = torch.from_numpy(label_dist.reshape(-1))
        if random_meta_class_set.sum().item() not in [0, num_class]:
            break
    #print(random_meta_class_set)
    return random_meta_class_set

def build_meta_class_set_from_partitions_per_batch(partitionings, targets ,num_class, num_partitions):
    """ meta-class sets from partitions randomly selected per batch
    """

    meta_class_set_idx = np.random.randint(len(partitionings))
    # meta_class_set_idx = batch_idx % len(manual_meta_class_sets)
    meta_class_set = torch.LongTensor(partitionings[meta_class_set_idx])
    #print('meta class set for this batch: ', meta_class_set)
    meta_labels = torch.LongTensor([meta_class_set[i] for i in targets.data])
    return meta_class_set, meta_labels
    # meta_labels = torch.LongTensor(meta_labels)
