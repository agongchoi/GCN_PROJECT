...
def build_set(root, year, train, noise_rate=0.5, noise_path=''):
    """
    ...
    if year == 2010:
        ...
        if train:
            ...
            noises = [(noises[idx][0], labels[idx]) for idx in idxs]
        else:
            noises = []
...
class CUB200(data.Dataset):
    ...
    def __init__(self, root, year, train=True, transform=None, target_transform=None, download=False,
                 loader=default_loader, noise_rate=0.5, noise_path=''):
        ...
        self.imgs, self.classes, self.class_to_idx = build_set(os.path.join(self.root, self.raw_folder),
                                                               self.year, self.train, noise_rate, noise_path)
