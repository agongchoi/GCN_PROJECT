...
def build_set(root, year, train, noise_rate=0.5):
    """
    ...
    if year == 2010:
        ...
        if train:
            ...
            n_noises = int(len(noises) * noise_rate)
            with temp_seed(0):
                idxs = np.random.permutation(len(noises))[:n_noises]
                labels = np.random.randint(0, n_target_classes, n_noises)
            selected_data = [(noises[idx][0], noises[idx][1]) for idx in idxs]
            train_noises = [(noises[idx][0], label) for idx, label in zip(idxs, labels)]
            noises = [(noises[idx][0], label) for idx, label in zip(idxs, labels)]

            for i in range(100):
                print('original target is : ', selected_data[i][1], 'noised target is : ', train_noises[i][1])
        else:
            noises = []

        imgs = [(y[0], y[1]) for y in filter(lambda x: x[2], imgs)]

        return imgs+noises, classes, class_to_idx
...
class CUB200(data.Dataset):
    ...
    def __init__(self, root, year, train=True, transform=None, target_transform=None, download=False,
                 loader=default_loader, noise_rate=0.5):
        ...
        self.imgs, self.classes, self.class_to_idx = build_set(os.path.join(self.root, self.raw_folder),
                                                               self.year, self.train, noise_rate)
