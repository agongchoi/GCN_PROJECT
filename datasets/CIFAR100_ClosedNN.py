...
def build_set(root, train, imgs, noise_path='', noise_rate=0.5):
    ...
    if train:
        with temp_seed(0):
            ...
            noisy_labels = torch.load(noise_path)
            with temp_seed(0):
                is_noisy = np.random.multinomial(1, [1 - noise_rate, noise_rate], len(imgs)).argmax(1)
            for i, n in enumerate(is_noisy):
                if n == 1:
                    imgs[i] = (imgs[i][0], noisy_labels[i])



        for i in range(100):
            print('original target is : ', tmp_imgs[i][1], 'noised target is : ', imgs[i][1])
