...
def main():
    ...
    if args.dataset == 'cub200':
        ...
        data_dir = '/data2/geeho/datasets/imagenet'
    ...
    if args.net_type == 'resnet':
        ...
        torch.save(_model.state_dict(), 'model.pkl')
        model = resnet18_glore(pretrained=False)
        state_dict = torch.load('model.pkl')
        model.load_state_dict(state_dict, strict=False)
        model.fc = nn.Linear(_model.fc.in_features, cf.num_classes[args.dataset])
        # _model = models.resnet18(pretrained=True)
        # feat_dim = _model.fc.in_features
        # #_model.fc = nn.Linear(_model.fc.in_features, cf.num_classes[args.dataset])
    ...
    #
    # glore1 = GloRe(feat_dim, int(feat_dim /2), 128)
    # glore2 = GloRe(feat_dim, int(feat_dim /2), 128)
    #
    # model.add_module('glore1', glore1)
    # model.add_module('glore2', glore2)
    #
    # model.add_module('classifier', nn.Linear(feat_dim, cf.num_classes[args.dataset]))
